...
class Post(DBWrapper):
    ...
    def upload(self):
        self.cursor.execute('''
            INSERT INTO POST(username, title, content, posted, picture, profile_pic, likes) VALUES (%s,%s,%s,%s,%s,%s,%s);

        ''', (self.username, self.title, self.content, self.posted, self.picture, self.profile_pic, self.likes))
